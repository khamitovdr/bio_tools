...
class TestConcurrentDeviceAccess:
    ...
    async def test_single_device_concurrent_operations(self, monkeypatch):
        ...
        async def simple_operation(op_id: str):
            """Perform a simple device operation."""
            async with pump:
                await pump.set_default_flow_rate(5.0)
                await pump.pour_volume(0.05)  # Small volume for faster test
                completed_operations.append(op_id)
                return op_id

        # Start multiple operations concurrently
        # The device lock will ensure they don't interfere with each other
        tasks = [
            simple_operation("op1"),
            simple_operation("op2"),
            simple_operation("op3"),
        ]
    ...
    async def test_multiple_devices_concurrent_operations(self, monkeypatch):
        ...
        async def timed_operation(pump, pump_id: str):
            ...
            async with pump:
                ...
                # Pour a smaller volume for faster test: 0.05mL at 5mL/min = 0.6s
                await pump.pour_volume(0.05)
        ...
        # Each operation: 0.1s sleep + 0.6s pour = 0.7s per device
        # If concurrent, should be ~0.7-1.0s total; if sequential, would be ~2.1s
