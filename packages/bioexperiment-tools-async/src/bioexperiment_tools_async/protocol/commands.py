...
class PumpCommand(DeviceCommand):
    ...
    def identification(cls) -> "PumpCommand":
        ...
        return cls(data=[1, 2, 3, 4, 181], response_length=4, description="Pump identification")
...
class SpectrophotometerCommand(DeviceCommand):
    ...
    def identification(cls) -> "SpectrophotometerCommand":
        ...
        return cls(data=[1, 2, 3, 4, 0], response_length=4, description="Spectrophotometer identification")

    @classmethod
    def get_temperature(cls) -> "SpectrophotometerCommand":
        """Create get temperature command."""
        return cls(data=[76, 0, 0, 0, 0], response_length=4, description="Get temperature")

    @classmethod
    def start_measurement(cls) -> "SpectrophotometerCommand":
        """Create start measurement command."""
        return cls(data=[78, 4, 0, 0, 0], response_length=0, description="Start optical density measurement")

    @classmethod
    def get_measurement_result(cls) -> "SpectrophotometerCommand":
        """Create get measurement result command."""
        return cls(data=[79, 4, 0, 0, 0], response_length=4, description="Get measurement result")
